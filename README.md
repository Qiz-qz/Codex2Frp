...
release/v1.4.18/             Latest public installer release
...
```text
release/v1.4.18/Codex2FrpSetup-v1.4.18.exe
```

Verify the installer with:

```text
release/v1.4.18/SHA256SUMS.txt
```
...
```powershell
Codex2FrpSetup-v1.4.18.exe --silent --install-dir E:\Codex2Frp
```
...
- Gives ChatGPT computer-use activity a dedicated, privacy-safe public identity instead of exposing it as a generic Node.js tool.
- Makes same-turn guidance acknowledgement atomic and preserves the authoritative turn link needed for the mobile waiting-response state to clear on first real progress.
- Corresponds to internal backend 2.4.18; the public release contains backend assets only and never includes CodexHM packages or source.
