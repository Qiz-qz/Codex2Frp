...
test('browser frontend uses standalone assets instead of legacy inline monolith', () => {
  ...
  assert.match(html, /href="styles\/codex2frp-console\.css\?v=1\.0\.0"/, 'HTML loads the versioned standalone browser stylesheet');
  assert.match(html, /src="scripts\/codex2frp-client\.js\?v=1\.0\.0"/, 'HTML loads the versioned standalone browser client script');
