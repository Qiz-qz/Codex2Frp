...
function normalizeControlOverrides(value = {}, isThreadId = () => false) {
  ...
  const reasoning = typeof value.reasoning === 'string' && ['low', 'medium', 'high', 'xhigh', 'max', 'ultra'].includes(value.reasoning.trim())
  ...
  const timestamp = field => typeof value[field] === 'string' ? value[field].trim() : '';
  const modelUpdatedAt = timestamp('modelUpdatedAt') || (model ? updatedAt : '');
  const reasoningUpdatedAt = timestamp('reasoningUpdatedAt') || (reasoning ? updatedAt : '');
  const speedUpdatedAt = timestamp('speedUpdatedAt') || (speed ? updatedAt : '');
  return {
    model, reasoning, speed, threadId, updatedAt,
