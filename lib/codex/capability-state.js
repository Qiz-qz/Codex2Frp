...
function createConfirmedControls(service) {
  ...
  controls.updateThreadSettings = async (params = {}, control = {}) => {
    ...
    const confirmed = await service.updateThreadSettings(params, control);
    const settings = copy(params) || {};
    delete settings.threadId;
    const target = confirmed && confirmed.target && typeof confirmed.target === 'object'
      ? copy(confirmed.target)
      : null;
    return {
      status: 'confirmed',
      operation: 'thread.settings',
      observation: {
        source: target && target.source === 'desktopInternalRpc' ? 'desktopInternalRpc' : 'confirmedRequest',
