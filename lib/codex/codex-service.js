...
function classification(kind, error, options = {}) {
  return {
    ...
    rpcCode: Number.isFinite(error && error.rpcCode) ? error.rpcCode : null,
...
function classifyCodexServiceError(error, options = {}) {
  ...
  const rpcCode = Number.isFinite(error && error.rpcCode) ? error.rpcCode : null;
  ...
  if (code === 'APP_SERVER_RPC_TIMEOUT') {
    return mutation
