...
function isUnmaterializedThreadReadError(error) {
  const details = objectOrEmpty(error && error.details);
  const rpcError = objectOrEmpty(details.rpcError);
  const text = [
    error && error.message,
    rpcError.message,
    details.message,
  ].filter(Boolean).join(' ');
  return /thread\s+[^\s]+\s+is not materialized yet|includeTurns is unavailable before first user message/i
    .test(text);
}
...
function compactLatestEventSnapshot(result = {}) {
  if (!result || result.mode !== 'snapshot' || !Array.isArray(result.events)
    || !Array.isArray(result.turns) || result.turns.length === 0) return result;
  const latestTurn = result.turns[result.turns.length - 1];
  const latestTurnId = String(latestTurn && latestTurn.turnId || '');
  const latestPresentationId = String(latestTurn && latestTurn.presentationId || '');
  if (!latestTurnId) return result;
  const events = result.events.filter(event => event && (
    String(event.turnId || '') === latestTurnId
    || (latestPresentationId && String(event.presentationId || '') === latestPresentationId)
  ));
  // A compact snapshot may omit earlier cursor ranges, but its final event
  // must still anchor the authoritative response cursor for the mobile
  // client's snapshot validator. Keep that one record if the latest turn did
  // not itself advance the global feed cursor.
  if (result.cursor > 0 && !events.some(event => event.cursor === result.cursor)) {
    const cursorEvent = result.events.find(event => event && event.cursor === result.cursor);
    if (cursorEvent) events.push(cursorEvent);
  }
  return { ...result, events, turns: [latestTurn] };
}
...
class V3ApiRouter {
  ...
  async handle(request = {}) {
    ...
    try {
      ...
      if (method === 'GET' && eventMatch) {
        ...
        let result = projectPublicEventFeed(await this.requireEventRuntime().read(
          threadId,
          eventCursorParams(url.searchParams),
        ));
        if (url.searchParams.get('scope') === 'latest') {
          result = compactLatestEventSnapshot(result);
        }
        return success(result);
      }
  ...
  async flushQueue(threadId, body) {
    ...
    try {
      ...
      const classified = classifyCodexServiceError(error);
      const confirmedUnmaterialized = classified.kind === 'invalidRequest'
        && isUnmaterializedThreadReadError(error);
      if ((!locallyStartedEmpty || classified.kind !== 'invalidRequest')
        && !confirmedUnmaterialized) throw error;
