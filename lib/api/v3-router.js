...
class V3ApiRouter {
  ...
  async handle(request = {}) {
    ...
    try {
      ...
      if (method === 'GET' && eventSnapshotMatch) {
        ...
        return success(await this.requireEventRuntime().snapshot(threadId));
      }
      ...
      if (method === 'GET' && eventMatch) {
        ...
        return success(await this.requireEventRuntime().read(
          threadId,
      ...
      if (method === 'GET' && pendingRequestListMatch) {
        ...
        return success({
          threadId,
          items: this.requirePendingRequestStore().list(threadId),
        });
      }
          ...
          () => this.requireQueue().convertToSteer(item.id, {
            ...
            steer: async queued => {
              const inputRouter = new TurnInputRouter({
                queue: this.queue,
                getActiveTurn: async targetThreadId => {
                  const status = await this.readThreadStatus(targetThreadId);
                  return status.activeTurn || {
  ...
  async readThreadStatus(threadId) {
    ...
    return deriveThreadStatus(response, threadId);
  }
