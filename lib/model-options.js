...
const HIDDEN_DESKTOP_REASONING_EFFORTS = new Set(['max']);

function reasoningOptionFromDesktopMenuText(text = '') {
  const compact = cleanText(text).replace(/\s+/g, '').toLowerCase();
  if (!compact) return null;
  if ((compact.includes('更快消耗使用额度') || compact.includes('更快消耗额度')) &&
      (compact.includes('极高') || compact.includes('超高'))) {
    return { key: 'ultra', value: 'ultra', label: '极高', displayName: '极高', description: '更快消耗使用额度' };
  }
  if (compact === '轻度' || compact === '低') {
    return { key: 'low', value: 'low', label: '轻度', displayName: '轻度' };
  }
  if (compact === '中') return { key: 'medium', value: 'medium', label: '中', displayName: '中' };
  if (compact === '高') return { key: 'high', value: 'high', label: '高', displayName: '高' };
  if (compact === '极高' || compact === '超高') {
    return { key: 'xhigh', value: 'xhigh', label: '极高', displayName: '极高' };
  }
  return null;
}
...
function reasoningOptionsForModel(model = {}, targets = {}, liveOptions = []) {
  ...
  const efforts = (supported.length
    ...
    .filter(effort => !HIDDEN_DESKTOP_REASONING_EFFORTS.has(effort));
...
function modelSupportsSpeed(model = {}, catalogOptions = []) {
  ...
  return candidates.some(key => key === 'gpt56sol' || key === '56sol'
    || key === 'gpt55' || key === 'gpt54' || key === '55' || key === '54');
}

function speedOptionsForModel(model = {}, catalogOptions = [], targets = {}) {
  if (!modelSupportsSpeed(model, catalogOptions)) return [];
  const standard = targets.standard || {
    key: 'standard', value: 'default', serviceTier: 'default', label: '标准', displayName: '标准',
  };
  const fast = targets.fast || {
    key: 'fast', value: 'priority', serviceTier: 'priority', label: '高速', displayName: '高速',
  };
  // Codex catalog metadata lists only additional tiers. The ordinary/default
  // service tier is implicit and must remain selectable whenever speed is.
