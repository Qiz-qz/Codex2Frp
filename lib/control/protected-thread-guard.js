...
class ProtectedThreadGuard {
  ...
  assertAllowed(context = {}) {
    ...
    const protectedCandidates = action === 'control.enable'
      ? []
      : action === 'thread.openDesktop'
        ? [threadId]
        : [threadId, observedThreadId];
    const protectedCandidate = protectedCandidates.find(candidate => this.isProtected(candidate));
    ...
    if (mode === 'ui'
      ...
      && this.isProtected(desktopThreadId)) {
