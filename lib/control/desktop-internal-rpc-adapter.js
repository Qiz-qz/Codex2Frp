...
class DesktopInternalRpcAdapter {
  ...
  async updateThreadSettings(params = {}, control = {}) {
    const result = await this.invoke(DESKTOP_RPC_METHODS.updateThreadSettings, params, control);
    if (!this.onSettingsConfirmed) return result;
    const target = await this.onSettingsConfirmed(objectOrEmpty(params), result);
