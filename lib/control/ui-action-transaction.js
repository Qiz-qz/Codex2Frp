...
class UiActionTransaction {
  ...
  async run(context = {}, operation) {
    ...
    return GLOBAL_UI_ACTION_LOCK.run(async () => {
      ...
      const focusedWindowCaptureSupported = typeof this.adapter.getFocusedWindow === 'function';
      let originalFocusedWindow = null;
      if (focusedWindowCaptureSupported) {
        try {
          originalFocusedWindow = this.adapter.getFocusedWindow();
        } catch (error) {
          throw uiError(
            'UI_FOCUS_CAPTURE_FAILED',
            'The focused window could not be captured for the explicit UI action.',
            { action: context.action, threadId: context.threadId },
            { cause: error },
          );
        }
      ...
      try {
        ...
        if (!readOnly) {
          // Opening an explicit task is the one mutating action that is valid from
          // Codex's native home route, where there intentionally is no active task
          // to wait for.  The requested destination is still checked by the guard
          // before discovery and again here under the global UI lock.
          let currentGuardContext;
          if (context.action === 'thread.openDesktop') {
            try {
              currentGuardContext = await lockedGuardContext(context, this.resolveObservedThread);
            } catch (error) {
              if (!error || error.code !== 'UI_ACTIVE_THREAD_UNKNOWN') throw error;
              currentGuardContext = { ...context, mode: 'ui', observedThreadId: '', desktopThreadId: '' };
            }
          } else {
            currentGuardContext = await readyGuardContext(context, this.resolveObservedThread, {
      ...
      let focusedWindowRestored = !originalFocusedWindow;
      const focusRestoreErrors = [];
      if (originalFocusedWindow && !restoreResult.userFocusChanged) {
        if (typeof this.adapter.setFocusedWindow !== 'function') {
          focusedWindowRestored = false;
          focusRestoreErrors.push('focused-window-unavailable');
        } else {
          try {
            focusedWindowRestored = this.adapter.setFocusedWindow(originalFocusedWindow) !== false;
          } catch {
            focusedWindowRestored = false;
          }
          if (!focusedWindowRestored) focusRestoreErrors.push('focused-window');
        }
      }
      ...
      if ((restoreResult.restoreErrors && restoreResult.restoreErrors.length > 0)
        ...
        throw uiError(
          ...
          'The explicit UI action finished, but the original window state could not be fully restored.',
          {
            ...
            focusedWindowRestored,
            userFocusChanged: restoreResult.userFocusChanged,
            restoreErrors: [...(restoreResult.restoreErrors || []), ...focusRestoreErrors],
          },
