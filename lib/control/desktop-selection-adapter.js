...
function routeThreadId(value) {
  ...
  try {
    ...
    if (url.protocol === 'codex:' && url.hostname === 'threads') {
      ...
      && url.pathname === '/index.html' && /^#\/?(?:threads|local)\//i.test(url.hash)) {
      path = url.hash.slice(1).replace(/^([^/])/, '/$1');
    }
    const match = path.match(/^\/(?:threads|local)\/([0-9a-f-]+)\/?$/i);
...
class DesktopSelectionAdapter {
  constructor(options = {}) {
    ...
    this.lastObservationAvailable = false;
  }

  async observeDesktopSelection(raw, options = {}) {
    const evidence = raw === undefined ? await this.observeSource() : raw;
    this.lastObservationAvailable = Boolean(evidence && typeof evidence === 'object');
  ...
  async confirmSelection(requestedThreadId) {
    ...
    while (true) {
      let observed = null;
      try {
        observed = await this.observeDesktopSelection(undefined, { consumeEcho: false });
      } catch {
        observed = null;
      }
  ...
  async openDesktopThread(value) {
    ...
    return this.transaction.run({
      ...
      try {
        const navigation = await this.navigate({ threadId: requestedThreadId, sourceToken });
        let confirmation = await this.confirmSelection(requestedThreadId);
        const nativeDeepLinkConfirmed = navigation
          && navigation.method === 'codex-deep-link'
          && normalizeThreadId(navigation.confirmedThreadId) === requestedThreadId;
        if (confirmation.status === 'unavailable' && nativeDeepLinkConfirmed) {
          confirmation = {
            status: 'confirmed',
            requestedThreadId,
...
async function consumeDesktopSelectionForSync(adapter) {
  ...
  try { selection = await adapter.observeDesktopSelection(); } catch {
    return { selection: null, suppressed: false, observed: false, reason: 'observer_unavailable' };
  }
  if (!selection) return {
    selection: null,
    suppressed: false,
    observed: adapter.lastObservationAvailable === true,
    reason: 'selection_absent',
  };
  if (selection.suppressEcho === true) {
    return { selection: null, suppressed: true, observed: true, reason: 'phone_origin' };
  }
  return { selection, suppressed: false, observed: true };
}
