...
function exactSingleImageView(source) {
  const pattern = new RegExp(
    String.raw`^\s*const\s+${STATIC_IDENTIFIER_TOKEN}\s*=\s*await\s+tools\.view_image\s*\(\s*\{\s*(?:path|["']path["'])\s*:\s*(${STATIC_STRING_TOKEN})(?:\s*,\s*(?:detail|["']detail["'])\s*:\s*${STATIC_STRING_TOKEN})?\s*,?\s*\}\s*\)\s*;?\s*image\s*\(\s*([A-Za-z_$][A-Za-z0-9_$]*)\.image_url(?:\s*,\s*${STATIC_STRING_TOKEN})?\s*\)\s*;?\s*$`,
  );
