...
const SHELL_COMMAND_MARKERS = ['tools.shell_command', 'tools.exec_command'];
...
function validShellCommandValue(key, value) {
  if (key === 'cmd' || key === 'command' || key === 'justification' || key === 'shell' || key === 'workdir') {
    return value.kind === 'string';
  }
  if (key === 'sandbox_permissions') {
    return value.kind === 'string' && ['use_default', 'require_escalated'].includes(value.value);
  }
  if (key === 'login' || key === 'tty') return value.kind === 'primitive' && typeof value.value === 'boolean';
  if (key === 'timeout_ms' || key === 'yield_time_ms' || key === 'max_output_tokens') {
...
function readExactShellCommandCall(source, start) {
  const marker = SHELL_COMMAND_MARKERS.find(value => source.startsWith(value, start));
...
function readResultConsumer(source, start, assignment) {
  ...
  if (source.startsWith('.output', cursor)) cursor = skipWhitespace(source, cursor + '.output'.length);
  if (source[cursor] !== ')') return undefined;
  cursor = skipWhitespace(source, cursor + 1);
...
function readStaticMappedCommands(source) {
  ...
  if (!source.startsWith('=>', cursor)) return undefined;
  cursor = skipWhitespace(source, cursor + 2);
  const marker = SHELL_COMMAND_MARKERS.find(value => source.startsWith(value, cursor));
  if (!marker) return undefined;
  cursor = skipWhitespace(source, cursor + marker.length);
  if (source[cursor] !== '(') return undefined;
  cursor = skipWhitespace(source, cursor + 1);
