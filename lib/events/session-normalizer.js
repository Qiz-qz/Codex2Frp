...
const { isStrictInternalEnvironmentContext } = require('./internal-environment-context');
const {
...
function extractPublicUserMessageText(content) {
  if (typeof content === 'string') return isStrictInternalEnvironmentContext(content) ? '' : content;
  if (!Array.isArray(content)) return '';
  return content
    .map(item => item && (item.text || item.message || ''))
    .filter(text => text && !isStrictInternalEnvironmentContext(text))
    .join('\n');
}
...
function exactApplyPatchChanges(input) {
  const patchText = extractExactApplyPatch(input);
  if (!patchText) return [];
...
function exactApplyPatchSummaryRows(payload = {}, item) {
  ...
  const changes = exactApplyPatchChanges(payload.input);
...
function toolSummary(payload = {}, item) {
  ...
  if (type === 'commandExecution' || type === 'local_shell_call' || rawName === 'shell_command'
    ...
    const displayDetail = type === 'commandExecution' ? commandExecutionDisplayDetail(payload) : '';
...
function createSessionNormalizer(options = {}) {
  ...
  function userMessage(textValue, item, turnId = '', attachments = [], representation = '') {
    if (isStrictInternalEnvironmentContext(textValue)) return null;
  ...
  function normalizeEventMessage(item) {
    ...
    if (payload.type === 'user_message') {
      return userMessage(payload.message || '', item, '', localImageAttachments(payload.local_images), 'event_msg');
    }
  ...
  function normalizeResponseItem(item) {
    ...
    if (payload.type === 'message') {
      if (payload.role === 'user') {
        ...
        return userMessage(extractPublicUserMessageText(payload.content), item, turnId, [], 'response_item');
      }
