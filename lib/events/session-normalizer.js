...
function toolSummary(payload = {}, item) {
  ...
  if (type === 'error') {
    ...
    const retryAttempt = Number(payload.retryAttempt);
    const retryLimit = Number(payload.retryLimit);
    const reconnectTitle = Number.isInteger(retryAttempt) && retryAttempt > 0
      && Number.isInteger(retryLimit) && retryLimit > 0
      ? `正在重新连接 ${Math.min(retryAttempt, retryLimit)}/${retryLimit}`
      : '连接异常，正在重试';
    return summary('tool', payload.willRetry === true ? reconnectTitle : '执行出现错误', item,
      { ...common, toolKind: 'error', state: errorState, willRetry: payload.willRetry === true,
        ...(payload.noticeKind === 'reconnect' ? { noticeKind: 'reconnect' } : {}),
        ...(Number.isInteger(retryAttempt) && retryAttempt > 0 ? { retryAttempt } : {}),
        ...(Number.isInteger(retryLimit) && retryLimit > 0 ? { retryLimit } : {}),
        ...(body ? { body } : {}) });
  }
...
function publicToolSurfaceKind(payload = {}) {
  const result = payload.result && typeof payload.result === 'object' ? payload.result : {};
  const resolved = result.Ok && typeof result.Ok === 'object' ? result.Ok : result;
  const meta = resolved._meta && typeof resolved._meta === 'object' ? resolved._meta : {};
  const surface = meta['codex/toolSurface'] && typeof meta['codex/toolSurface'] === 'object'
    ? meta['codex/toolSurface'] : {};
  return surface.kind === 'computerUse' ? 'computerUse' : '';
}
...
function createSessionNormalizer(options = {}) {
  ...
  const userPresentationBySource = new Map();
  ...
  function pairedUserSource(text, timestampMs, representation, recordId = '') {
    ...
    if (previous && complementaryPair
      ...
      return { sourceKey: previous.sourceKey, paired: true };
    }
    ...
    return { sourceKey, paired: false };
  }
  function explicitlyGuidedClientId(value = '') {
    return /^mobile-steer-/i.test(String(value || '').trim());
  }
  function explicitlyQueuedClientId(value = '') {
    return /^mobile-queue-/i.test(String(value || '').trim());
  }
  function userMessage(textValue, item, turnId = '', attachments = [], representation = '') {
    ...
    const recordId = String(item.payload?.client_id || item.payload?.id || '');
    const paired = pairedUserSource(text, timestampMs, representation, recordId);
    const explicitGuided = explicitlyGuidedClientId(recordId);
    const explicitQueued = explicitlyQueuedClientId(recordId);
    const existingPresentation = userPresentationBySource.get(paired.sourceKey);
    const effectiveTurnId = String(turnId || existingPresentation?.turnId
      || (explicitGuided ? turnState.activeTurnId : '') || '');
    const showGuidedBadge = explicitGuided || existingPresentation?.showGuidedBadge === true;
    const shouldClassify = !existingPresentation
      || (!existingPresentation.turnId && Boolean(effectiveTurnId));
    const delivery = showGuidedBadge
      ? 'steer'
      : explicitQueued || existingPresentation?.explicitQueued === true
        ? 'queued'
        : shouldClassify && effectiveTurnId
          ? classifyUserDelivery(turnState, effectiveTurnId)
          : existingPresentation?.delivery || 'initial';
    const counted = existingPresentation?.counted === true
      || (turnState.active === true && effectiveTurnId === turnState.activeTurnId);
    userPresentationBySource.set(paired.sourceKey, {
      delivery, showGuidedBadge, explicitQueued: explicitQueued || existingPresentation?.explicitQueued === true,
      turnId: effectiveTurnId, counted,
    });
    if (counted && existingPresentation?.counted !== true) {
      turnState = observeUserDelivery(turnState, effectiveTurnId);
    }
    if (userPresentationBySource.size > 256) {
      userPresentationBySource.delete(userPresentationBySource.keys().next().value);
    }
    return {
      schemaVersion: 2,
      type: 'message',
      role: 'user',
      text,
      sourceKey: paired.sourceKey,
      turnId: effectiveTurnId,
      delivery,
      showGuidedBadge,
      ...(recordId ? { clientRequestId: recordId } : {}),
      time: item.timestamp || '',
  ...
  function normalizeEventMessage(item) {
    ...
    if (payload.type === 'mcp_tool_call_end') {
      ...
      return summary('tool', title, item, {
        ...
        ...(publicToolSurfaceKind(payload) ? { surfaceKind: publicToolSurfaceKind(payload) } : {}),
        ...(durationMs !== undefined ? { durationMs } : {}),
      });
    }
