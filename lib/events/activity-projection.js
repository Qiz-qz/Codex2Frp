...
const { copyPrivateAttachmentSource, privateAttachmentIdentity } = require('./private-attachment-source');
...
function safeFileLabel(value) {
  const label = String(value || '').replace(/[\u0000-\u001F\u007F]/g, '').trim().slice(0, 512);
...
function aggregateTitle(activity) {
  ...
  if (activity.kind === 'shell') title = `已运行 ${count} 条命令`;
    ...
    title = `已生成 ${count} 张图像`;
  } else if (activity.kind === 'image') title = `已查看 ${count} 张图像`;
...
function safeAttachmentUrl(value) {
  ...
  return /^\/codex\/attachment\/[A-Za-z0-9_-]{32}$/.test(source) ? source : '';
}
...
function safeAttachment(item) {
  ...
  return Object.keys(safe).length > 0 ? copyPrivateAttachmentSource(item, safe) : null;
}

function mergeSafeImageAttachments(target, incoming) {
  const merged = Array.isArray(target) ? target : [];
  const seen = new Set(merged.map((item, index) => privateAttachmentIdentity(item, index)));
  for (const [ordinal, item] of (incoming || []).entries()) {
    const safe = safeAttachment(item);
    if (!safe) continue;
    const key = privateAttachmentIdentity(safe, ordinal);
    if (!seen.has(key)) {
      merged.push(safe);
      seen.add(key);
    }
...
function sanitizePublicAttachments(incoming) {
  return mergeSafeImageAttachments([], incoming);
}
...
function safeActivity(activity) {
  ...
  if (activity.kind !== 'subagent') {
    if (activity.kind === 'shell' || activity.kind === 'file' || activity.kind === 'web') {
      const displayDetail = safeDisplayDetail(activity.displayDetail);
      if (displayDetail) result.displayDetail = displayDetail;
    }
    ...
    if (activity.kind === 'mcp' && activity.surfaceKind === 'computerUse') result.surfaceKind = 'computerUse';
    ...
    if (activity.kind === 'file') {
      const fileLabel = safeFileLabel(activity.fileLabel);
      const changeKind = String(activity.changeKind || '').toLowerCase();
      if (fileLabel) result.fileLabel = fileLabel;
      if (SAFE_CHANGE_KINDS.has(changeKind)) result.changeKind = changeKind;
    }
  }
  if (Number.isFinite(activity.count)) result.count = activity.count;
  if (activity.kind !== 'subagent' && Number.isSafeInteger(activity.sourceOrdinal) && activity.sourceOrdinal > 0) {
    result.sourceOrdinal = activity.sourceOrdinal;
  }
...
function appendProjectedActivity(process, activity) {
  ...
  let summary = process.activities[process.activities.length - 1];
  if (!summary || summary.aggregateKey !== key) summary = null;
  if (!summary) {
    const segment = process.activities.filter(item => item.aggregateKey === key).length + 1;
    summary = {
      ...projected,
      id: `${process.turnId || 'turn'}-summary-${key.replace(/:$/, '')}${segment > 1 || key === 'image:imageView' ? `-${segment}` : ''}`,
      aggregateKey: key,
      count: 0,
      failedCount: 0,
      cancelledCount: 0,
