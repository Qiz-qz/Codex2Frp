...
const LEAF_PAIR_PATTERN = /<(cwd|shell|current_date|timezone|root|file_system|agent|special)(?:\s[^<>]*)?>[^<>]*<\/\1>/g;
const SELF_CLOSING_PATTERN = /<(?:cwd|shell|current_date|timezone|root|file_system|agent|special|entry|filesystem|workspace_roots|permission_profile|subagents)(?:\s[^<>]*)?\/>/g;
const CONTAINER_TAG_PATTERN = /<\/?(?:entry|file_system|filesystem|workspace_roots|permission_profile|subagents)(?:\s[^<>]*)?>/g;
...
function isStrictRecommendedPluginsContext(value) {
  const text = String(value || '').replace(/\r\n/g, '\n').trim();
  return /^<recommended_plugins>\nHere is a list of plugins that are available but not installed\.[\s\S]*\n<\/recommended_plugins>$/.test(text);
}

function isStrictAgentsInstructionsContext(value) {
  const text = String(value || '').replace(/\r\n/g, '\n').trim();
  return /^# AGENTS\.md instructions for [^\n]+\n\n<INSTRUCTIONS>\n[\s\S]*\n<\/INSTRUCTIONS>$/.test(text);
}

function isStrictInternalUserContext(value) {
  return isStrictInternalEnvironmentContext(value)
    || isStrictRecommendedPluginsContext(value)
    || isStrictAgentsInstructionsContext(value);
}

module.exports = {
  isStrictInternalEnvironmentContext,
  isStrictInternalUserContext,
};
