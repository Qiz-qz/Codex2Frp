...
function eventIdFor(event) {
  if ((event.summaryKind === 'reasoning' || event.summaryKind === 'plan') && event.turnId) {
    return crypto.createHash('sha256').update(JSON.stringify({
      type: 'summary', summaryKind: event.summaryKind, turnId: event.turnId,
    })).digest('hex');
  }
  if (event.sourceKey) {
    return crypto.createHash('sha256').update(JSON.stringify({
...
function canonicalUserEvent(event = {}) {
  return event.type === 'message' && event.role === 'user'
    && Boolean(String(event.turnId || '').trim());
}
...
class EventReconciler {
  ...
  incrementalRollout(notification, sequence) {
      ...
      if (rawType === 'custom_tool_call' && rawItem.name === 'exec') {
        ...
        return {
          ...
          payload: {
            ...
            status: rawItem.status, input,
            internal_chat_message_metadata_passthrough: metadata,
          },
        };
      }
      if (rawType === 'custom_tool_call' && rawItem.name === 'apply_patch') {
        const input = typeof rawItem.input === 'string' && rawItem.input.length <= 64 * 1024
          ? rawItem.input : '';
        return {
          type: 'response_item', timestamp,
          payload: {
            type: 'custom_tool_call', name: 'apply_patch', call_id: callId,
            status: rawItem.status, input,
            internal_chat_message_metadata_passthrough: metadata,
          },
        };
    ...
    if (method === 'error' || ['warning', 'guardianWarning', 'configWarning', 'deprecationNotice'].includes(method)) {
      ...
      if (method === 'error') {
        ...
        const retryAttempt = willRetry ? (this.reconnectAttempts.get(turnId) || 0) + 1 : 0;
        if (willRetry) this.reconnectAttempts.set(turnId, retryAttempt);
        else this.reconnectAttempts.delete(turnId);
        return {
          type: 'response_item', timestamp, _eventSourceKey: sourceKey,
          payload: {
            type: 'error', id: sourceKey, message: 'Codex reported an error.',
            willRetry,
            ...(willRetry ? { retryAttempt: Math.min(retryAttempt, 5), retryLimit: 5, noticeKind: 'reconnect' } : {}),
            internal_chat_message_metadata_passthrough: metadata,
          },
        };
      }
  ...
  async ingestRpcNotification(notification = {}) {
    ...
    const params = notification.params || {};
    const lifecycleTurnId = notificationTurnId(notification);
    if ((notification.method === 'turn/started' || notification.method === 'turn/completed') && lifecycleTurnId) {
      this.reconnectAttempts.delete(lifecycleTurnId);
    }
    if (['item/started', 'item/completed'].includes(notification.method)
  ...
  turnSnapshot() {
    return [...this.turnStates.entries()]
      .map(([turnId, state]) => {
        const presentationRevision = typeof this.feed.turnRevision === 'function'
          ? this.feed.turnRevision(turnId)
          : '';
        return {
          turnId,
          state,
