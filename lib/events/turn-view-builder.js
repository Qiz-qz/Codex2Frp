...
function canonicalUserEvent(event = {}) {
  return Boolean(String(event.turnId || '').trim());
}
...
function coalescePairedUserEvents(events) {
  ...
  for (const event of Array.isArray(events) ? events : []) {
    ...
    result[index] = {
      ...
      ...((preferred.showGuidedBadge === true || supplement.showGuidedBadge === true)
        ? { showGuidedBadge: true } : {}),
      ...((preferred.clientRequestId || supplement.clientRequestId)
        ? { clientRequestId: String(preferred.clientRequestId || supplement.clientRequestId) } : {}),
...
function replaceProcessActivities(process, activities) {
  process.activities = [];
...
function mergedDetailActivities(turns) {
  const merged = [];
  for (const turn of turns) {
    for (const activity of Array.isArray(turn.process?.detailActivities)
      ? turn.process.detailActivities : []) {
...
function mergeGuidedTurnDetails(turns) {
  const groups = new Map();
  for (const turn of Array.isArray(turns) ? turns : []) {
    const taskTurnId = String(turn.process?.turnId || turn.turnId || '').trim();
    if (!taskTurnId) continue;
    const siblings = groups.get(taskTurnId) || [];
    siblings.push(turn);
    groups.set(taskTurnId, siblings);
  }
  for (const siblings of groups.values()) {
    if (siblings.length < 2) continue;
    const owner = siblings[siblings.length - 1];
    const timeline = dedupeAdjacentDesktopNarratives(orderPublicTimeline(
      siblings.flatMap(turn => Array.isArray(turn.timeline) ? turn.timeline : []),
    ), owner.turnId);
    owner.timeline = timeline;
    owner.segments = segmentVisibleTimeline(timeline);
    if (owner.process) replaceProcessActivities(owner.process, mergedDetailActivities(siblings));
  }
  return turns;
}
...
function userMessage(event, threadId) {
  ...
  if (id) message.id = id;
  if (event.delivery) message.delivery = event.delivery;
  message.showGuidedBadge = event.showGuidedBadge === true;
  if (message.showGuidedBadge) {
    message.kind = 'steer';
    message.label = '已引导';
  }
  if (event.clientRequestId) message.clientRequestId = String(event.clientRequestId);
...
function buildTurnViews(events, threadId = '') {
  ...
  for (const [eventIndex, event] of coalescePairedUserEvents(events).entries()) {
    ...
    if (event.type === 'message' && event.role === 'user') {
      ...
      // Current ChatGPT can emit task_started before the primary user record.
      // Delivery is correctly "initial" for that first message, so attach any
      // matching primary input to the empty live shell before steer handling.
      if (current && current.turnId === protocolTurnId && !current.user) {
        current.user = message;
        if (message.id) current.presentationId = message.id;
        continue;
      }
    ...
    if (event.type === 'summary') {
      ...
      for (const timelineEntry of activityEntries) {
        ...
        if (kind === 'subagent') {
          ...
          if (kind === 'file') {
            const fileLabel = timelineEntry?.fileLabel ?? event.fileLabel;
            const changeKind = timelineEntry?.changeKind ?? event.changeKind;
            if (typeof fileLabel === 'string') activity.fileLabel = fileLabel;
            if (typeof changeKind === 'string') activity.changeKind = changeKind;
          }
          ...
          const surfaceKind = timelineEntry?.surfaceKind ?? event.surfaceKind;
          if (kind === 'mcp' && surfaceKind === 'computerUse') activity.surfaceKind = 'computerUse';
  ...
  return mergeGuidedTurnDetails(turns);
}
