...
class AppServerRuntime {
  constructor(options = {}) {
    ...
    this.serverRequestSink = typeof options.serverRequestSink === 'function'
      ...
      : null;
    this.serverRequestLifecycleSink = typeof options.serverRequestLifecycleSink === 'function'
      ? options.serverRequestLifecycleSink
      : null;
    this.closedRequestEpochs = new Set();
    ...
    this.capabilityOptions = {
      ...
      supportedMethods: Array.isArray(options.supportedMethods)
        ...
        : [...(Array.isArray(this.schemaProfile.requestMethods)
          ? this.schemaProfile.requestMethods
    ...
    this.confirmedNativeControls = options.confirmedNativeControls === true;
  ...
  getMeta() {
    return {
      ...
      capabilities: createCapabilityManifest({
        ...this.capabilityOptions,
        runtimeReady: this.state === 'ready' && Boolean(this.service),
      }),
    };
  }
  ...
  async startOwnedService(lifecycleGeneration) {
    ...
    try {
      ...
      this.service = this.confirmedNativeControls ? createConfirmedControls(service) : service;
      this.state = 'ready';
      return this.service;
    } catch (error) {
  ...
  notifyRequestEpochClosed(connectionEpoch) {
    const epoch = Number(connectionEpoch);
    if (!Number.isSafeInteger(epoch) || epoch <= 0 || this.closedRequestEpochs.has(epoch)) return false;
    this.closedRequestEpochs.add(epoch);
    if (this.serverRequestLifecycleSink) {
      try {
        Promise.resolve(this.serverRequestLifecycleSink({
          type: 'connectionClosed', connectionEpoch: epoch,
        })).catch(() => {});
      } catch {}
    }
    return true;
  ...
  stop(signal = 'SIGTERM') {
    ...
    this.service = null;
    this.detachNotificationClient();
    this.notifyRequestEpochClosed(connectionEpoch);
  ...
  handleExit(event = {}) {
    ...
    this.lifecycleGeneration += 1;
    this.notifyRequestEpochClosed(epoch);
    this.detachNotificationClient();
    this.activeStart = null;
    this.service = null;
    this.pid = null;
